...
// logos by ticker symbol (via /logos/symbol).
...
export default function CompanyLogo({ ticker, name, size = 64, src }: Props) {
  const sym = (ticker ?? '').trim().toUpperCase()
  const resolved =
    src ||
    (sym ? `https://assets.parqet.com/logos/symbol/${encodeURIComponent(sym)}?size=128` : '')
