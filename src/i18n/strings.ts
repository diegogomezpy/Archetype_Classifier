...
const en = {
  ...
  admin: {
    ...
    autofillHint: 'Ticker or ISIN → auto-fills the details. Crypto works out of the box; equities need an API key (below).',
    autofillFilled: (n: number) => `Filled ${n} ${n === 1 ? 'field' : 'fields'} from market data.`,
    autofillNeedId: 'Enter a ticker or ISIN first.',
    autofillNoKey: 'Set an equity API key in “Market data source” above (crypto needs no key).',
    ...
    autofillNetwork: 'Fetch failed — check the key and connection, then try again.',
    // Data-source settings
    dataSourceTitle: 'Market data source',
    dataSourceHint:
      'Crypto autofills via CoinGecko (no key). For equities/ETFs, add a free API key. Note: ATM implied vol and bond/structured data have no free source and stay manual.',
    provider: 'Equity provider',
    providerNone: 'None (manual only)',
    apiKey: 'API key',
    apiKeyHint: 'Free key from financialmodelingprep.com. Stored in this browser only.',
    saveSettings: 'Save',
    settingsSaved: 'Saved',
  ...
  adminArch: {
    ...
    mixTitle: 'Recommended asset mix',
    mixHint:
      'The model portfolio shown on the advisor dashboard for each archetype. Percentages are normalized to 100 on save.',
    total: 'Total',
...
const es: UIStrings = {
  ...
  admin: {
    ...
    autofillHint: 'Ticker o ISIN → autocompleta los detalles. Cripto funciona sin clave; las acciones necesitan una clave API (abajo).',
    ...
    autofillNoKey: 'Configura una clave API de acciones en «Fuente de datos» arriba (cripto no necesita clave).',
    ...
    autofillNetwork: 'Falló la obtención — revisa la clave y la conexión, e intenta de nuevo.',
    // Ajustes de fuente de datos
    dataSourceTitle: 'Fuente de datos de mercado',
    dataSourceHint:
      'Cripto se autocompleta vía CoinGecko (sin clave). Para acciones/ETFs, agrega una clave API gratuita. Nota: la vol. implícita ATM y los datos de bonos/estructurados no tienen fuente gratuita y quedan manuales.',
    provider: 'Proveedor de acciones',
    providerNone: 'Ninguno (solo manual)',
    apiKey: 'Clave API',
    apiKeyHint: 'Clave gratuita de financialmodelingprep.com. Se guarda solo en este navegador.',
  ...
  adminArch: {
    ...
    mixTitle: 'Mezcla de activos recomendada',
    mixHint:
      'El portafolio modelo que se muestra en el panel del asesor para cada arquetipo. Los porcentajes se normalizan a 100 al guardar.',
    total: 'Total',
